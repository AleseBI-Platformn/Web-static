...
const Navbar: React.FC<NavbarProps> = ({ onMenuClick }) => {
  ...
  const toggleDropdown = (menuId: number) => {
    const newOpenDropdowns = new Set<number>();
    
    // Si el menú clickeado no está abierto, abrirlo (cerrar todos los demás)
    if (!openDropdowns.has(menuId)) {
      newOpenDropdowns.add(menuId);
    }
    // Si ya está abierto, cerrarlo (dejar el set vacío)
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 xl:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center flex-1 justify-center mx-4">
            <div className="flex items-center space-x-1 xl:space-x-2 overflow-x-auto scrollbar-hide max-w-full">
              {menus.length === 0 ? (
                  ...
                  return (
                    <div key={menu.idmenu} className="relative navbar-dropdown flex-shrink-0">
                      ...
                      >
                        ...
                        {hasChildren && <span className="text-xs text-red-500 ml-1">({menu.children?.length})</span>}
                      </button>

                      {/* Desktop Dropdown Menu - CORREGIDO */}
                      {hasChildren && isOpen && (
                        <div className="absolute top-full left-0 mt-1 w-56 bg-white shadow-lg border border-gray-200 rounded-md z-[1000]">
                          <div className="py-1">
                            {menu.children?.map(child => (
                              <button
                                key={child.idmenu}
                                onClick={() => handleMenuClick(child)}
                                className="w-full flex items-center space-x-2 px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors duration-200"
                              >
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            ...
            <div className="relative navbar-dropdown">
              ...
              {/* User Dropdown */}
              {userDropdownOpen && (
                <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-[1000]">
                  <div className="py-1">
